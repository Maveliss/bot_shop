...
async def process_add_product(message: Message):
    ...
    await message.answer('Название товара?', reply_markup=markup)
...
async def process_cancel(message: Message, state: FSMContext):
    ...
    # markup.add(add_product)
    # markup.add(delete_category)
    # await message.answer('Хотите что-нибудь добавить или удалить?', add_product, delete_category)
...
async def process_confirm(message: Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        tag = db.fetchone('SELECT title FROM categories WHERE idx = ?',
        ...
        await state.finish()
        await message.answer('☑️ Готово', reply_markup=ReplyKeyboardRemove())
        await process_settings(message)
...
async def process_confirm_back(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        await message.answer(f"Изменить цену товара с <b>{data['price']}</b>?",
                             reply_markup=back_markup())


@dp.message_handler(IsAdmin(), content_types=ContentType.TEXT,
                    state=ProductState.image)
async def process_image_change(message: Message, state: FSMContext):
    if message.text == back_message:
        ...
        await message.answer('Пришлите фото товара') # не работает!!!!
...
async def process_confirm_invalid(message: Message, state: FSMContext):
    await message.answer('Я тебя не понимаю, не ломай бота!')
